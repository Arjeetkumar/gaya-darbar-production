...
async function runAnalyticsTests() {
  ...
  let createdOrder1: IOrder | null = null;
  let createdOrder2: IOrder | null = null;
  ...
  try {
    ...
    analyticsAuthCheck({ user: customerUser } as any, {} as any, (err?: any) => {
      if (err && err.statusCode === 403) custDenied = true;
    });

    let kitchenDenied = false;
    analyticsAuthCheck({ user: kitchenUser } as any, {} as any, (err?: any) => {
      if (err && err.statusCode === 403) kitchenDenied = true;
    });

    let riderDenied = false;
    analyticsAuthCheck({ user: riderUser } as any, {} as any, (err?: any) => {
      if (err && err.statusCode === 403) riderDenied = true;
    });

    let adminAllowed = false;
    analyticsAuthCheck({ user: adminUser } as any, {} as any, (err?: any) => {
      if (!err) adminAllowed = true;
    });

    let managerAllowed = false;
    analyticsAuthCheck({ user: managerUser } as any, {} as any, (err?: any) => {
