...
async function runPhase9ProductionTests() {
  ...
  let testOrder: IOrder | null = null;
  ...
  try {
    ...
    let healthResJson: any = null;
    const mockResHealth: any = {
      status: (code: number) => {
        return {
          json: (data: any) => {
    ...
    getHealthCheck({} as any, mockResHealth);
    ...
    getReadinessCheck({} as any, mockResReadiness);
    ...
    globalErrorHandler(secretError, {} as any, mockErrRes, (() => {}) as any);
    ...
    const updatedPayment1 = await Payment.findOneAndUpdate(
      { _id: payment._id, refundedAmount: 0 },
      { $inc: { refundedAmount: 300 } },
      { new: true }
    );
    ...
    const updatedPayment2 = await Payment.findOneAndUpdate(
      { _id: payment._id, refundedAmount: 300 },
      { $inc: { refundedAmount: payment.amount - 300 } },
      { new: true }
    );
    if (!updatedPayment2) {
