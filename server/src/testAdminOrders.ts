...
async function runAdminOrderTests() {
  ...
  try {
    ...
    adminAuthCheck({ user: testCustomer } as any, {} as any, (err?: any) => {
      if (err && err.statusCode === 403) customerDenied = true;
    });

    let kitchenDenied = false;
    adminAuthCheck({ user: testKitchenStaff } as any, {} as any, (err?: any) => {
      if (err && err.statusCode === 403) kitchenDenied = true;
    });

    let adminAllowed = false;
    adminAuthCheck({ user: testAdmin } as any, {} as any, (err?: any) => {
