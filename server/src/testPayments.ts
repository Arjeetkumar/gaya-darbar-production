...
async function runPaymentTests() {
  ...
  let testOrder1: IOrder | null = null;
  let testOrder2: IOrder | null = null;
  ...
  try {
    ...
    adminCheck({ user: customer1 } as any, {} as any, (err?: any) => {
      if (err && err.statusCode === 403) custDeniedAdmin = true;
    });
    if (!custDeniedAdmin) {
    ...
    const updatedP1 = await Payment.findOneAndUpdate(
      { _id: paymentRec1._id, refundedAmount: currentRefunded },
      { $inc: { refundedAmount: partialAmount1 } },
      { new: true }
    );
    ...
    const updatedP2 = await Payment.findOneAndUpdate(
      { _id: paymentRec1._id, refundedAmount: currentRefunded2 },
      { $inc: { refundedAmount: partialAmount2 } },
      { new: true }
    );
