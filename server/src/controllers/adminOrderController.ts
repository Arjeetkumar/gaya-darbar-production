...
import { OrderStatus, OrderType, PaymentStatus } from '../models/Order.js';
...
export async function getAdminOrdersHandler(
  ...
  try {
    const { page, limit, search, status, orderType, paymentStatus, dateFrom, dateTo, sort } =
      req.query;
    ...
    const result = await getAdminOrders({
      ...
      orderType: typeof orderType === 'string' ? (orderType as OrderType) : undefined,
      paymentStatus: typeof paymentStatus === 'string' ? (paymentStatus as PaymentStatus) : undefined,
...
export async function updateAdminOrderStatusHandler(
  ...
  try {
    ...
    const updatedOrder = await updateAdminOrderStatus(orderId, status as OrderStatus, {
...
export async function cancelAdminOrderHandler(
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> {
