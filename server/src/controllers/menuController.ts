...
export async function getMenu(
  ...
  try {
    ...
    if (goal && typeof goal === 'string' && !VALID_GOALS.includes(goal)) {
      throw new AppError(
        `Invalid goal parameter '${goal}'. Valid goals: ${VALID_GOALS.join(', ')}`,
        400
      );
    }
    ...
    if (diet && typeof diet === 'string' && !VALID_DIETS.includes(diet)) {
      throw new AppError(
        `Invalid diet parameter '${diet}'. Valid diets: ${VALID_DIETS.join(', ')}`,
        400
      );
    }
    ...
    if (category && typeof category === 'string' && !VALID_CATEGORIES.includes(category)) {
      throw new AppError(
        `Invalid category parameter '${category}'. Valid categories: ${VALID_CATEGORIES.join(', ')}`,
        400
      );
    }
    ...
    if (sort && typeof sort === 'string' && !VALID_SORTS.includes(sort)) {
      throw new AppError(
        `Invalid sort parameter '${sort}'. Valid sorts: ${VALID_SORTS.join(', ')}`,
        400
      );
    }
