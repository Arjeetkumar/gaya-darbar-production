...
async function runSeed() {
  ...
  if (config.isProduction && !process.argv.includes('--force-prod-seed')) {
    logger.error(
      'CRITICAL SAFETY PRECAUTION: Seeding script aborted! Cannot run destructive database seed in PRODUCTION mode without --force-prod-seed flag.'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    logger.info(
      `Inserting ${initialMealBuilderOptions.length} initial Meal Builder Options...`
    );
    const insertedOptions = await MealBuilderOption.insertMany(
      initialMealBuilderOptions
    );
    logger.info(
      `Successfully seeded ${insertedOptions.length} Meal Builder Options.`
    );
