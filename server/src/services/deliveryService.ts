...
export interface GetAdminDeliveriesParams {
  page?: number;
  limit?: number;
  search?: string;
...
export interface AdminDeliveriesPaginatedResult {
 ...
 */
export async function createDeliveryForOrder(order: IOrder): Promise<IDelivery | null> {
 ...
 */
export async function getAvailableRiders(): Promise<IUser[]> {
  return User.find({
    role: 'delivery_rider',
    isActive: true,
    isDeleted: { $ne: true },
...
export async function getAdminDeliveries(
  params: GetAdminDeliveriesParams = {}
): Promise<AdminDeliveriesPaginatedResult> {
 ...
 */
export async function getDeliveryById(deliveryIdentifier: string): Promise<IDelivery> {
  const query: Record<string, unknown> = {};
...
export async function assignDeliveryRider(
  ...
  delivery.rider = rider._id as unknown as mongoose.Types.ObjectId;
  ...
  try {
    if (linkedOrder) {
      ...
      await createOrderNotification(
        ...
        `Rider ${rider.name} has been assigned to deliver order ${linkedOrder.orderNumber}.`
      );
    }
  } catch (notifErr) {
 ...
 */
export async function reassignDeliveryRider(
  deliveryIdentifier: string,
  newRiderId: string
): Promise<IDelivery> {
 ...
 */
export async function cancelDelivery(deliveryIdentifier: string): Promise<IDelivery> {
  const delivery = await getDeliveryById(deliveryIdentifier);
 ...
 */
export async function getMyDeliveryById(
  deliveryIdentifier: string,
  riderUserId: string
): Promise<IDelivery> {
  const delivery = await getDeliveryById(deliveryIdentifier);
...
export async function updateRiderDeliveryStatus(
  ...
  const ALLOWED_RIDER_TRANSITIONS: Record<DeliveryStatus, DeliveryStatus[]> = {
    unassigned: [],
  ...
  if (!allowedNext.includes(newStatus)) {
    throw new AppError(
      `Invalid rider status transition from '${currentStatus}' to '${newStatus}'. Allowed: [${
    ...
    try {
      ...
      if (newStatus === 'picked_up') {
        await createOrderNotification(
          ...
          'DELIVERY_PICKED_UP',
          'Order Picked Up',
          `Your order ${linkedOrder.orderNumber} was picked up by rider and is on its way!`
        );
      } else if (newStatus === 'out_for_delivery') {
        await createOrderNotification(
          linkedOrder.user,
          ...
          'Out for Delivery',
          `Rider is out for delivery with order ${linkedOrder.orderNumber}.`
        );
      } else if (newStatus === 'delivered') {
        await createOrderNotification(
          linkedOrder.user,
          ...
          'ORDER_DELIVERED',
          'Order Delivered',
          `Order ${linkedOrder.orderNumber} has been delivered successfully. Enjoy!`
        );
      } else if (newStatus === 'failed') {
        await createOrderNotification(
          linkedOrder.user,
          ...
          `Rider reported an issue delivering order ${linkedOrder.orderNumber}: ${failureReason}`
        );
      }
    } catch (notifErr) {
