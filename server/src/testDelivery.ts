...
async function runDeliveryTests() {
  ...
  let customerUser: IUser | null = null;
  let adminUser: IUser | null = null;
  let managerUser: IUser | null = null;
  let kitchenUser: IUser | null = null;
  let riderUser1: IUser | null = null;
  let riderUser2: IUser | null = null;
  let createdDeliveryOrder: IOrder | null = null;
  ...
  try {
    ...
    adminDispatchCheck({ user: customerUser } as any, {} as any, (err?: any) => {
      if (err && err.statusCode === 403) custDenied = true;
    });

    let kitchenDenied = false;
    adminDispatchCheck({ user: kitchenUser } as any, {} as any, (err?: any) => {
      if (err && err.statusCode === 403) kitchenDenied = true;
    });

    let riderDenied = false;
    adminDispatchCheck({ user: riderUser1 } as any, {} as any, (err?: any) => {
      if (err && err.statusCode === 403) riderDenied = true;
    });

    let adminAllowed = false;
    adminDispatchCheck({ user: adminUser } as any, {} as any, (err?: any) => {
      if (!err) adminAllowed = true;
    });

    let managerAllowed = false;
    adminDispatchCheck({ user: managerUser } as any, {} as any, (err?: any) => {
    ...
    if (availableRiders.some((r) => (r.role as string) !== 'delivery_rider')) {
    ...
    if (
      deliveryRecord1.deliveryAddressSnapshot.addressLine1 !== '789 Fuel Street' ||
    ...
    const pickedUpDelivery = await updateRiderDeliveryStatus(
      deliveryRecord1._id.toString(),
      riderUser1._id.toString(),
      'picked_up'
    );
    ...
    const outForDeliveryDelivery = await updateRiderDeliveryStatus(
      deliveryRecord1._id.toString(),
      riderUser1._id.toString(),
      'out_for_delivery'
    );
    ...
    const deliveredDelivery = await updateRiderDeliveryStatus(
      deliveryRecord1._id.toString(),
      riderUser1._id.toString(),
      'delivered'
    );
    ...
    try {
      await updateRiderDeliveryStatus(
        deliveryForCancel!._id.toString(),
        riderUser1._id.toString(),
        'picked_up'
      );
    } catch (err: any) {
