...
async function runNotificationTests() {
  ...
  let customer1: IUser | null = null;
  let customer2: IUser | null = null;
  let adminUser: IUser | null = null;
  let riderUser: IUser | null = null;
  let menuItem: any = null;
  let deliveryOrder: IOrder | null = null;
  let dineInOrder: IOrder | null = null;
  ...
  try {
    ...
    if (!kTicket) throw new Error('Kitchen ticket missing for delivery order.');
    ...
    if (!orderPrepNotif) throw new Error('❌ TEST 7 FAILED: ORDER_PREPARING notification missing.');
    ...
    if (!orderReadyNotif) throw new Error('❌ TEST 8 FAILED: ORDER_READY notification missing.');
    ...
    if (!deliveryRec) throw new Error('Delivery record missing.');
    ...
    if (!deliveryAssignedNotif) throw new Error('❌ TEST 12 FAILED: DELIVERY_ASSIGNED notification missing.');
    ...
    if (!pickedUpNotif) throw new Error('❌ TEST 13 FAILED: DELIVERY_PICKED_UP notification missing.');
    ...
    if (!outForDelNotif) throw new Error('❌ TEST 9 FAILED: ORDER_OUT_FOR_DELIVERY notification missing.');
    ...
    if (!deliveredNotif) throw new Error('❌ TEST 10 FAILED: ORDER_DELIVERED notification missing.');
    ...
    if (!cancelledNotif) throw new Error('❌ TEST 11 FAILED: ORDER_CANCELLED notification missing.');
    ...
    const failOrder = await createOrder(customer1._id.toString(), {
      ...
      items: [{ itemType: 'STANDARD_ITEM', menuItemId: String((menuItem as any)._id), name: menuItem.name, quantity: 1 }],
    });
    ...
    if (!failedNotif) throw new Error('❌ TEST 14 FAILED: DELIVERY_FAILED notification missing.');
    ...
    if (cust1Notifs.length === 0) throw new Error('❌ TEST 1 FAILED: Customer 1 returned 0 notifications.');
    if (cust2Notifs.length !== 0) throw new Error('❌ TEST 2 FAILED: Customer 2 retrieved notifications belonging to Customer 1.');
    ...
    if (unreadCountBefore === 0) throw new Error('❌ TEST 16 FAILED: Unread count before read is 0.');
