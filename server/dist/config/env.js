...
export const config = {
    ...
    databaseUrl: process.env.DATABASE_URL || 'mongodb://localhost:27017/gaya_darbar',
...
export function validateEnv() {
    ...
    if (!config.jwtSecret || config.jwtSecret.includes('dev_mode'))
    ...
    if (config.isProduction && missing.length > 0) {
        throw new Error(`FATAL PRODUCTION ENVIRONMENT ERROR: Missing or insecure configuration for: [${missing.join(', ')}]. Production process halted.`);
    }
