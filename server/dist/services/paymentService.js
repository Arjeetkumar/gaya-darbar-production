 ...
 */
export async function createRazorpayPaymentOrder(orderId, userId) {
 ...
 */
export async function verifyPaymentSignature(userId, payload) {
    ...
    if (sigBuffer.length !== expectedBuffer.length || !crypto.timingSafeEqual(sigBuffer, expectedBuffer)) {
        ...
        await order.save();
        try {
            await createOrderNotification(order.user, order._id, order.orderNumber, 'PAYMENT_FAILED', 'Payment Failed', `Payment verification failed for order ${order.orderNumber}.`);
        }
        catch (notifErr) {
    ...
    await order.save();
    try {
        await createOrderNotification(order.user, order._id, order.orderNumber, 'PAYMENT_SUCCESS', 'Payment Successful', `Your payment of ₹${order.total.toFixed(2)} for order ${order.orderNumber} was successful!`);
    }
    catch (notifErr) {
 ...
 */
export async function processRazorpayWebhook(rawBuffer, signatureHeader) {
        ...
        if (providerOrderId) {
            ...
            if (payment && payment.status !== 'paid') {
                ...
                if (order && order.paymentStatus !== 'paid') {
                    order.paymentStatus = 'paid';
                    await order.save();
                    await createOrderNotification(order.user, order._id, order.orderNumber, 'PAYMENT_SUCCESS', 'Payment Confirmed', `Payment of ₹${order.total.toFixed(2)} for order ${order.orderNumber} confirmed via gateway.`);
                }
            }
        }
    }
        ...
        if (providerOrderId) {
            ...
            if (payment && payment.status === 'pending') {
                ...
                if (order && order.paymentStatus === 'pending') {
                    order.paymentStatus = 'failed';
                    await order.save();
                    await createOrderNotification(order.user, order._id, order.orderNumber, 'PAYMENT_FAILED', 'Payment Failed', `Gateway payment failed for order ${order.orderNumber}.`);
                }
            }
        }
    }
 ...
 */
export async function processAdminRefund(paymentId, requestedAmount, reason, adminUser) {
    ...
    if (requestedAmount > remainingRefundable) {
        throw new AppError(`Requested refund amount ₹${requestedAmount} exceeds remaining refundable balance of ₹${remainingRefundable}.`, 400);
    }
    // Trigger REFUND_INITIATED notification
    const order = await Order.findById(payment.order);
    if (order) {
        try {
            await createOrderNotification(order.user, order._id, order.orderNumber, 'REFUND_INITIATED', 'Refund Initiated', `A refund of ₹${requestedAmount.toFixed(2)} has been initiated for order ${order.orderNumber}.`);
        }
        catch (err) {
    ...
    const updatedPayment = await Payment.findOneAndUpdate({ _id: payment._id, refundedAmount: currentRefunded }, { $inc: { refundedAmount: requestedAmount } }, { new: true });
    if (!updatedPayment) {
    ...
    try {
        ...
        if (order) {
            try {
                await createOrderNotification(order.user, order._id, order.orderNumber, 'REFUND_FAILED', 'Refund Failed', `Refund attempt of ₹${requestedAmount.toFixed(2)} failed: ${razorErr.message || 'Gateway error'}`);
            }
            catch (err) {
    ...
    if (order) {
        ...
        try {
            await createOrderNotification(order.user, order._id, order.orderNumber, 'REFUND_COMPLETED', 'Refund Completed', `Refund of ₹${requestedAmount.toFixed(2)} completed for order ${order.orderNumber}.`);
        }
        catch (err) {
