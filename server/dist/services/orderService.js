 ...
 */
const KITCHEN_STAFF_ALLOWED_ORDER_STATUSES = [
    'confirmed',
    'preparing',
    'ready',
    'completed',
 ...
 */
export function calculateServerFuelScore(protein, calories, fats, hasBase, hasProtein, vegCount) {
    let score = 65;
...
export async function createOrder(userId, input) {
    ...
    let validatedTableIdentifier = null;
    if (orderType === 'delivery') {
        ...
        if (!fullName || !phone || !addressLine1 || !city || !state || !postalCode) {
            throw new AppError('Delivery address requires fullName, phone, addressLine1, city, state, and postalCode.', 400);
        }
        sanitizedAddress = {
    ...
    for (const rawItem of items) {
        ...
        if (rawItem.itemType === 'STANDARD_ITEM') {
            ...
            if (!menuItemDoc || menuItemDoc.isDeleted) {
                throw new AppError(`Requested menu item '${rawItem.name || rawItem.menuItemId || rawItem.slug}' is invalid or no longer available.`, 400);
            }
            if (!menuItemDoc.isAvailable) {
                throw new AppError(`Menu item '${menuItemDoc.name}' is currently out of stock.`, 400);
            }
            ...
            const customOptionsSnapshot = optionDocs.map((optDoc) => ({
                optionId: String(optDoc._id),
                name: optDoc.name,
            ...
            const fuelScore = calculateServerFuelScore(protein, calories, fats, hasBase, hasProtein, vegCount);
            processedSnapshots.push({
    ...
    try {
        ...
        await createOrderNotification(newOrder.user, newOrder._id, newOrder.orderNumber, 'ORDER_PLACED', 'Fuel Order Placed', `Your order ${newOrder.orderNumber} has been placed successfully.`);
    }
    catch (notifErr) {
...
export async function getUserOrderById(userId, orderIdentifier) {
    const query = {};
 ...
 */
export async function updateOrderStatus(orderIdentifier, newStatus, actor) {
    ...
    if (!newStatus || !validOrderStatuses.includes(newStatus)) {
        throw new AppError(`Invalid order status '${newStatus}'. Must be one of: ${validOrderStatuses.join(', ')}.`, 400);
    }
    // Role Permissions Check
    if (actor.role === 'kitchen_staff' && !KITCHEN_STAFF_ALLOWED_ORDER_STATUSES.includes(newStatus)) {
        throw new AppError(`Role 'kitchen_staff' is not authorized to set order status to '${newStatus}'.`, 403);
    }
    // Find Order
    const query = {};
    if (mongoose.Types.ObjectId.isValid(orderIdentifier)) {
    ...
    if (!allowedTransitions.includes(newStatus)) {
        throw new AppError(`Invalid order status transition from '${currentStatus}' to '${newStatus}'. Allowed next status(es): [${allowedTransitions.join(', ') || 'none'}].`, 400);
    }
    // Apply update
    order.status = newStatus;
    await order.save();
    logger.info(`Order ${order.orderNumber} status transitioned: ${currentStatus} -> ${newStatus} by ${actor.role} (${actor.userId})`);
    // Synchronize corresponding KitchenTicket if present
    const validKitchenStatuses = ['pending', 'preparing', 'ready', 'completed', 'cancelled'];
    if (validKitchenStatuses.includes(newStatus)) {
        const targetKitchenStatus = newStatus;
        ...
        if (ticket && ticket.status !== targetKitchenStatus) {
            ...
            const now = new Date();
            if (targetKitchenStatus === 'preparing' && !ticket.startedAt)
                ticket.startedAt = now;
            if (targetKitchenStatus === 'ready' && !ticket.readyAt)
                ticket.readyAt = now;
            if (targetKitchenStatus === 'completed' && !ticket.completedAt)
                ticket.completedAt = now;
            await ticket.save();
            logger.info(`Synchronized KitchenTicket ${ticket._id} status to '${targetKitchenStatus}' from direct Order update.`);
        }
    }
    ...
    try {
        ...
        if (notifConfig) {
            await createOrderNotification(order.user, order._id, order.orderNumber, notifConfig.type, notifConfig.title, notifConfig.message);
        }
    }
    catch (notifErr) {
