 ...
 */
export async function getAdminOrderById(orderIdentifier) {
    const query = {};
 ...
 */
export async function updateAdminOrderStatus(orderIdentifier, newStatus, actor) {
 ...
 */
export async function cancelAdminOrder(orderIdentifier, actor) {
    const order = await getAdminOrderById(orderIdentifier);
    if (order.status === 'completed' || order.status === 'cancelled') {
        throw new AppError(`Order ${order.orderNumber} is already '${order.status}' and cannot be cancelled.`, 400);
    }
    order.status = 'cancelled';
    await order.save();
    logger.info(`Order ${order.orderNumber} cancelled by ${actor.role} (${actor.userId})`);
