 ...
 */
export async function getCustomerOrderTracking(orderIdentifier, userId) {
    const query = {};
    ...
    const delivery = await Delivery.findOne({ order: order._id })
        ...
        .exec();
    const riderObj = delivery && delivery.rider && typeof delivery.rider.name === 'string'
        ? delivery.rider
        : null;
    ...
    if (currentStatus === 'cancelled') {
        timeline.push({ type: 'ORDER_PLACED', label: 'Fuel Order Placed', status: 'completed', timestamp: order.createdAt }, { type: 'ORDER_CANCELLED', label: 'Order Cancelled', status: 'cancelled', timestamp: order.updatedAt });
    }
        ...
        const statusSequence = ['pending', 'confirmed', 'preparing', 'ready', 'completed'];
        const currentIdx = statusSequence.indexOf(currentStatus);
        timeline.push({
            type: 'ORDER_PLACED',
            label: 'Fuel Order Placed',
