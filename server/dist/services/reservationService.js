 ...
 */
export async function checkReservationAvailability(reservationDate, timeSlot, partySize) {
...
export async function createReservation(userId, input) {
    ...
    const availability = await checkReservationAvailability(reservationDate, timeSlot, partySize);
    if (!availability.isAvailable) {
        throw new AppError(`No table available for ${partySize} guests at ${timeSlot} on ${reservationDate}. Please select another time slot.`, 400);
    }
    let assignedTable = availability.matchingTables[0];
    if (tableId && mongoose.Types.ObjectId.isValid(tableId)) {
        const requestedTable = availability.matchingTables.find((t) => t._id.toString() === tableId);
        if (requestedTable) {
...
export async function getUserReservationById(userId, reservationId) {
...
export async function cancelUserReservation(userId, reservationId) {
