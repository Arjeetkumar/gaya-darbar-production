...
export async function getTableByQrIdentifier(qrCodeIdentifier) {
    if (!qrCodeIdentifier) {
 ...
 */
export async function validateTableForDineIn(tableIdentifier) {
    if (!tableIdentifier) {
    ...
    if (!table) {
        throw new AppError(`Table '${tableIdentifier}' does not exist or is currently inactive.`, 400);
    }
    return table;
}
/**
 ...
 */
export async function seedTablesIfEmpty() {
