 ...
 */
const TICKET_TO_ORDER_STATUS_MAP = {
    preparing: 'preparing',
    ready: 'ready',
    completed: 'completed',
    cancelled: 'cancelled',
 ...
 */
export async function createKitchenTicketFromOrder(order) {
    if (!order || !order._id) {
 ...
 */
export async function getKitchenTickets(filters = {}) {
    const query = {};
    if (filters.status) {
 ...
 */
export async function getKitchenTicketById(id) {
 ...
 */
export async function updateKitchenTicketStatus(id, newStatus, actorInfo) {
    const validStatuses = [
        'pending',
        'preparing',
        'ready',
    ...
    if (!allowedNextStatuses.includes(newStatus)) {
        throw new AppError(`Invalid status transition from '${currentStatus}' to '${newStatus}'. Allowed next status(es): [${allowedNextStatuses.join(', ') || 'none'}].`, 400);
    }
    ...
    logger.info(`Kitchen ticket ${ticket._id} status transitioned: ${currentStatus} -> ${newStatus} by actor: ${actorInfo?.role || 'system'} (${actorInfo?.userId || 'system'})`);
    ...
    if (targetOrderStatus) {
        ...
        logger.info(`Synchronized Order ${ticket.orderNumber} status to '${targetOrderStatus}' from kitchen ticket update.`);
    }
    ...
    try {
        ...
        if (linkedOrder) {
            ...
            if (newStatus === 'preparing') {
                await createOrderNotification(linkedOrder.user, linkedOrder._id, linkedOrder.orderNumber, 'ORDER_PREPARING', 'Kitchen Preparing', `Our chefs are now preparing order ${linkedOrder.orderNumber}.`);
            }
            else if (newStatus === 'ready') {
                await createOrderNotification(linkedOrder.user, linkedOrder._id, linkedOrder.orderNumber, 'ORDER_READY', 'Order Ready', `Order ${linkedOrder.orderNumber} is prepared and ready!`);
            }
        }
    }
    catch (notifErr) {
 ...
 */
export async function setKitchenTicketPriority(id, priority) {
    const validPriorities = ['normal', 'high', 'urgent'];
    if (!priority || !validPriorities.includes(priority)) {
        throw new AppError(`Invalid priority '${priority}'. Must be one of: ${validPriorities.join(', ')}.`, 400);
    }
    const ticket = await getKitchenTicketById(id);
