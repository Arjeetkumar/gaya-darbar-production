...
export function generateToken(user) {
    return jwt.sign({ id: user._id.toString(), role: user.role }, config.jwtSecret, { expiresIn: config.jwtExpiresIn });
}
export async function registerCustomer(input) {
    ...
    if (fitnessGoal && !VALID_GOALS.includes(fitnessGoal)) {
        throw new AppError(`Invalid fitnessGoal parameter '${fitnessGoal}'. Valid options: ${VALID_GOALS.join(', ')}`, 400);
    }
    // Validation: Dietary Preference
    if (dietaryPreference && !VALID_DIETS.includes(dietaryPreference)) {
        throw new AppError(`Invalid dietaryPreference parameter '${dietaryPreference}'. Valid options: ${VALID_DIETS.join(', ')}`, 400);
    }
