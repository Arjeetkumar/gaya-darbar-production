import { getUserNotifications, getUnreadNotificationCount, markNotificationAsRead, markAllNotificationsAsRead, } from '../services/notificationService.js';
export async function getUserNotificationsHandler(req, res, next) {
    try {
        ...
        const notifications = await getUserNotifications(req.user._id.toString(), limit ? Number(limit) : 20);
        res.status(200).json({
            success: true,
            data: notifications,
        });
