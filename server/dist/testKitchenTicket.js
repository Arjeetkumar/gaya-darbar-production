...
import { createKitchenTicketFromOrder, getActiveKitchenTickets, updateKitchenTicketStatus, } from './services/kitchenTicketService.js';
...
async function runKitchenTicketTests() {
    ...
    try {
        ...
        const mockReq = { user: testCustomer };
        const mockRes = {};
        const mockNext = (err) => {
        ...
        const ticketPreparing = await updateKitchenTicketStatus(createdTicket._id.toString(), 'preparing', { userId: testKitchenStaff._id.toString(), role: 'kitchen_staff' });
        ...
        const ticketReady = await updateKitchenTicketStatus(createdTicket._id.toString(), 'ready', { userId: testKitchenStaff._id.toString(), role: 'kitchen_staff' });
        ...
        const ticketCompleted = await updateKitchenTicketStatus(createdTicket._id.toString(), 'completed', { userId: testKitchenStaff._id.toString(), role: 'kitchen_staff' });
        ...
        if (kitchenTicketSchemaPaths.includes('total') ||
